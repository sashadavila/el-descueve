...
export default function AdminNotifications() {
    ...
    const generateNotifications = async () => {
        try {
            ...
            await api.notifications.generate(users)
            loadNotifications()
        } catch (error) {
    ...
    // Actualizar contador en el header cuando cambian las notificaciones
    useEffect(() => {
        const updateUnreadCount = async () => {
            try {
                const { count } = await api.notifications.getUnreadCount()
                // Actualizar el badge en el localStorage o evento
    ...
    const markAsRead = async (id) => {
        try {
            ...
            // Mover notificación de no leída a leída
            ...
            loadNotifications() // Recargar para mantener consistencia
        } catch (error) {
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                {currentNotifications.length === 0 ? (
                    ...
                    currentNotifications.map(notification => (
                        <div
                            key={notification.id}
                            className={`bg-white rounded-xl border transition-all hover:shadow-md ${!notification.read && activeTab === 'unread'
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg text-center">
                <p className="text-xs text-gray-400">
                    ...
                    Las notificaciones marcadas como leídas se mueven a la sección de leídas.
                </p>
            </div>
        </div>
    )
